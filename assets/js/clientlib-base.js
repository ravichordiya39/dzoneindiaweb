...
function getQueryURL() {
    ...
    return "/bin/infosys/press-release?initialItemsCount="+initialItemsCount+"&ispr="+ispr+"&isevents="+isevents+"&url="+url;
}
...
//    }
    ...
    for (let i = 0; i < currentactive-1; i++) {
		...
		$('#items-group').children().first().append(lastPR);
    }
    for(i=0;i<pageObjs.length;i++)

    {
        ...
        if(pressReleaseItem && pressReleaseItem.length>0){
            ...
            if(socialTitle && $(pressReleaseItem).find('.socialTitle').length>0){
                ...
                $(socialTitle).attr('data-title',loadMoreHTMLTitle);
			//console.log("Load More Article Link"+loadMoreArticleLink);
                var urlofarticle;
          ...
          /*      if(loadMoreArticleLink != "null")
            ...
            if(pagereflink && $(pressReleaseItem).find('.press-releaseDownload').length>0){
                ...
                var socialShareHTML = $(pressReleaseItem).find('.press-release-date');
                /* Temporary commenting download icon html -NEED to add it later - START
                if(socialShareHTML.length >0){
                    socialShareHTML.after(downloadHTML);
                }
                ...
                Temporary commenting download icon html -NEED to add it later - END */
...
$("#articleYearFilter").on("change", function() {
    ...
    if($('#industriesFilter').val()!=null){
			appendUrlFilterIndex=appendUrlFilterIndex+"&industry="+$('#industriesFilter').val();
    }
    if($('#servicesFilter').val()!=null){
			appendUrlFilterIndex=appendUrlFilterIndex+"&service="+$('#servicesFilter').val();
    }
...
$("#industriesFilter").on("change", function() {
	...
	var queryURL = getQueryURL();
     if( getSelectIndexValue != null ){
        	UrlSlice = queryURL+"&industry="+getSelectIndexValue; 
			if(getSelectIndexValue == "All" ){
				var industry="&industry="+getSelectIndexValue;
                UrlSlice = UrlSlice.replace(industry, "");
			}
          appendUrlFilterIndex = UrlSlice;        
        }
    else{
...
$("#servicesFilter").on("change", function() {
	...
	var queryURL = getQueryURL();
    if( getSelectIndexValue != null ){
        	UrlSlice = queryURL+"&service="+getSelectIndexValue; 
			if(getSelectIndexValue == "All" ){
				var service="&service="+getSelectIndexValue;
                UrlSlice = UrlSlice.replace(service, "");
			}
          appendUrlFilterIndex = UrlSlice;        
        }
    else{
